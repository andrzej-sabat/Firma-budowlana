 ...
 * @author student
 */
public class PracownikZmianaHaslaController implements Initializable {
    ...
    @FXML public void bPotwierdz(ActionEvent event) {
        try {
            ...
            CallableStatement cstmt = connection.prepareCall("{call ZMIANA_HASLA_PRACOWNIK(?, ?)}");
            cstmt.setInt(1, Login.LoginController.p.getId());
            cstmt.setString(2, tfNoweHaslo.getText());
            if(tfNoweHaslo.getText().equals(tfNoweHaslo2.getText()) && tfStareHaslo.getText().equals(Login.LoginController.p.getHaslo())) {
                cstmt.execute();
